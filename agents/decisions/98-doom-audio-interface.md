# Decision 98: Extend the Guest When the Interface, Not the Frontend, Is What Is Missing

Status: **Accepted, 2026-09-08.**
`examples/doom` pins upstream doom.wasm v0.1.0 plus [one commit](https://github.com/sisshiki1969/doom.wasm/tree/sound) adding thirteen `audio` imports, and `ruby/gui/audio.rb` plays them; the other seven frontends answer those imports silently.
...
- Positive: the example gains the one capability its README had recorded as permanently missing, and gains it in the shape the example is about: thirteen imports, implementable in any of the seven languages.
...
- Negative: the pin is a fork branch rather than a release, because upstream has nothing to release yet, and `raw.githubusercontent.com` rather than a release asset because the session that built it could not create one.
  ...
  A wasm import cannot be omitted, so seven frontends carry thirteen imports they do not implement.
